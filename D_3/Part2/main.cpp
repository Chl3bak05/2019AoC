...
int find_time_to_intersect(vector<vector<char>> path1, vector<vector<int>> int1, vector<int> pos1, vector<vector<char>> path2, vector<vector<int>> int2, vector<int> pos2)
{
    int x = 0, y = 0;
    int counter = 0;
    while (true)
    {
        cout << x << "*" << y << endl;
        int1[pos1[0] + x][pos1[1] + y] = counter;
        if (path1[pos1[0] + x + 1][pos1[1] + y] == 'c')
        {
            x++;
        }
        if (path1[pos1[0] + x - 1][pos1[1] + y] == 'c')
        ...
        if (path1[pos1[0] + x][pos1[1] + y + 1] == 'c')
        {
            y++;
        }
        if (path1[pos1[0] + x][pos1[1] + y - 1] == 'c')
        ...
        counter++;
        if (pos2[0] + x < 0 || pos2[0] + x >= path2.size() || pos2[1] + y < 0 || pos2[1] + y >= path2[pos2[0]].size())
            continue;
        if (path1[pos1[0] + x][pos1[1] + y] == 'c' && path2[pos2[0] + x][pos2[1] + y] == 'c')
            break;
    }
    return counter;
}
...
int main()
{
    ...
    vector<vector<int>> int_tab1(tab1.size(), vector<int>(tab1[0].size(), 0));
    vector<vector<int>> int_tab2(tab2.size(), vector<int>(tab2[0].size(), 0));

    //cout << find_distance_to_intersect(tab1, point1, tab2, point2);
    cout << find_time_to_intersect(tab1, int_tab1, point1, tab2, int_tab2, point2) + find_time_to_intersect(tab2, int_tab2, point2, tab1, int_tab1, point1);
